...
class SplitDialog < GenericDialog

  def initialize( libglade=nil )
    super( "splitdialog", libglade )
  end
  ...
  def response_ok( args )
    ret = []
    ret << @libglade["splitdialog_x"].value.to_i
    ret << @libglade["splitdialog_y"].value.to_i
    ret << @libglade["splitdialog_note"].active?
    ret
  end

  def setup( args )
    @libglade["splitdialog_x"].value = 2
    @libglade["splitdialog_y"].value = 1
  end
