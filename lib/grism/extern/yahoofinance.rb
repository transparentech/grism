...
#     "i5" => [ "orderBook", "val" ],
  ...
  #
  def YahooFinance.get( symbols, format, tout=DEFAULT_READ_TIMEOUT )
    ...
    #puts "h=#{proxy.host};p=#{proxy.port};u=#{proxy.user};p=#{proxy.password}"

#     Net::HTTP.Proxy( proxy.host, proxy.port, 
#                      proxy.user, proxy.password ).start( "finance.yahoo.com",
#                                                          80 ) do |http|
#       http.read_timeout = tout
#       response = nil
...
#    realhost = "finance.yahoo.com"
    realhost = "download.finance.yahoo.com"
#    puts "YF.get :start proxy"
    http = Net::HTTP.Proxy( proxy.host, 
                            proxy.port, 
                            proxy.user, 
                            proxy.password ).new( realhost, 80 )
#    puts "YF.get :got proxy"
#    http.set_debug_output $stderr
    http.open_timeout = $PREFS['internet.open_timeout'].to_i
#    puts "YF.get :starting"
    http.start() do |conn|
#      puts "YF.get :in proxy"
      conn.read_timeout = $PREFS['internet.read_timeout'].to_i
      response = nil
#      puts "YF.get :getting"
      response = conn.get( "/d/quotes.csv?s=#{symbols}&f=#{format}&e=.csv" )
#      puts "YF.get :got response"
      return "" if !response
#      puts "YF.get :chomping"
      return response.body.chomp
    end
#    puts "YF.get :end timeout"
  end
